function validate() {
    ...
    function validateFormHandler(e) {
        ...
        let passwordAreOk = passwordIsValid && passwordInput.value === confirmPasswordInput.value;
        setBorder(passwordInput, passwordAreOk);
        setBorder(confirmPasswordInput, passwordAreOk);
        ...
        let mainIntpuAreValid = usernameIsValid && emailIsValid && passwordAreOk;
        let companyInfoIsValid = !isCompanyCheckbox.checked || (isCompanyCheckbox.checked && companyNumberIsValid);
        let shouldSlowValidDiv = mainIntpuAreValid && companyInfoIsValid;
        validDiv.style.display = shouldSlowValidDiv ? 'block' : 'none';
    }
